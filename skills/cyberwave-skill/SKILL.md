...
- Call `cw_list_primitives` to see curated catalog asset aliases (e.g. `camera`, `lidar`). These are catalog assets, not procedural environment geometry, and can be added directly by alias via `cw_add_twin_to_environment` — no need to search the full catalog first.
...
- **Use `cw_list_primitives` before `cw_search_catalog`** — when the user wants to add common sensors or generic catalog objects (camera, lidar, etc.) to an environment, call `cw_list_primitives` first. These are curated public catalog assets with a short alias and can be added immediately without a search round-trip.
